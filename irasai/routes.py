...
from .forms import utility_password_check, RegistracijosForma, PaskyrosAtnaujinimoForma, SlaptazodzioAtnaujinimoForma, IrasasForm, PrisijungimoForma, UzklausosAtnaujinimoForma
...
def registruotis():
    ...
    if form.validate_on_submit():
        ...
        patvirtintas_slaptazodis = form.patvirtintas_slaptazodis.data

        if not utility_password_check(slaptazodis):
            flash("Slaptažodis neatitinka reikalavimų.", 'danger')
            return render_template("registruotis.html", form=form)

        if slaptazodis != patvirtintas_slaptazodis:
            flash("Slaptažodžiai nesutampa.", 'danger')
            return render_template("registruotis.html", form=form)

        slaptazodis_hash = bcrypt.generate_password_hash(slaptazodis).decode('utf-8')
        vartotojas = Vartotojas(vardas=vardas, el_pastas=el_pastas, slaptazodis=slaptazodis_hash)
        db.session.add(vartotojas)
        db.session.commit()
